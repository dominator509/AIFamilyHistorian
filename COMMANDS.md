...
- Apply local/test migrations and provision the non-owner worker role (requires `WORKER_DATABASE_URL`): `corepack pnpm --filter @family-historian/database migrate`; verify schema invariants: `corepack pnpm --filter @family-historian/database verify`.
