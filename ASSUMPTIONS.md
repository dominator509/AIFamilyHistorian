...
| Host media binaries are available | EP-000 expected direct host probes | FFmpeg, ExifTool, ImageMagick, ClamAV and OCRmyPDF are absent from the current host PATH; a dedicated `worker-runtime` image now supplies the pinned toolchain | `sh scripts/media-tools-check.sh`; `docker build --target worker-runtime -t family-historian-worker:local .` plus the worker-image tool probe | No; host probe remains deferred while the pinned worker image is the approved execution surface |
