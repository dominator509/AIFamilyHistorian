...
- `sh scripts/test-integration.sh` -> `integration tests: ok` (11 files, 29 tests, including real PostgreSQL concurrent upload-reservation and archive-queue-capacity serialization, archive-to-organization context rejection, Redis distributed rate limiting and revocation persistence, derivative recipe uniqueness/fixity, archive-partition isolation, provider-authoritative billing status and subscription uniqueness, tenant-scoped privacy/deletion-hold evidence, export and narration review intake, SQL outbox claim/lease/completion/retry/dead-letter, unsupported-job, and stale-worker fencing proofs, and signed Stripe webhook persistence, replay, payload-hash mismatch, and tenant-metadata rejection)
