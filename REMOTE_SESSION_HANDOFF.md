...
- Current code checkpoint: the next commit in this run will include the Redis distributed rate-limiter hardening checkpoint; the exact full hash is recorded in Git after commit.
- Branch: `master`
...
- `sh scripts/test-integration.sh` -> `integration tests: ok` (6 files, 13 tests, including real Redis distributed rate limiting plus SQL outbox claim/lease/completion/retry/dead-letter, unsupported-job, and stale-worker fencing proofs)
...
| Worker and job execution | Dispatcher and media quarantine path implemented; remaining job families pending | SQL `FOR UPDATE SKIP LOCKED` claim/lease, UUID lease-token fencing, tenant transaction scope, bounded retry/backoff/dead-letter, unsupported-job fail-closed behavior, streamed object download/fixity, media plan execution, derivative persistence, and clean/error quarantine transitions | 12 real-database integration tests; worker image build/tool probe; media executor unit coverage | Privacy/export/narration/transcription handler families, hosted object-storage fixture, production queue topology and authenticated media fixture | Unimplemented job types terminal-fail safely; media fixture/live-fire still needs a representative object in the worker image |
...
- The worker now has a real SQL dispatcher with lease-token fencing and a media quarantine handler, but privacy, export, transcription, narration, and deletion job families still require implementation and fixture proofs. Unsupported job types fail closed rather than reporting success.
