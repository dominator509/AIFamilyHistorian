...
- `sh scripts/test-integration.sh` -> `integration tests: ok` (8 files, 17 tests, including real Redis distributed rate limiting and revocation persistence, transactional active-upload quota rejection, tenant-scoped privacy/deletion-hold evidence, export and narration review intake, and SQL outbox claim/lease/completion/retry/dead-letter, unsupported-job, and stale-worker fencing proofs)
...
- `sh scripts/restore-check.sh .artifacts/backups/family-historian-20260808T094958Z.dump.enc` -> `restore-check: ok`; the encrypted backup decrypted into a disposable database and reported `schema_migrations=7`.
