...
| Authentication and authorization | Local bearer/TOTP hardening complete; native auth/passkey rollout pending | Short-lived signed sessions, archive permission checks, owner-gated member changes, tenant-scoped rights subjects, pending-only rights/media/share boundaries, privacy-write authorization, tenant boundaries, cross-archive mutation checks, fail-closed visibility/rights checks, RFC-compatible TOTP enrollment/replay protection/recovery codes, Redis-backed distributed IP/principal/archive rate scopes | Unit 65, integration 15, E2E 10; TOTP RFC-vector, Redis limiter, principal/archive limiter, cross-scope rights/upload, and publication-boundary coverage | WebAuthn/passkey live-fire, database-backed session rotation/revocation/device management, byte/active-upload/queue quotas, production secret injection | Current API has no native login, cookie CSRF path, or session inventory |
