...
- `sh scripts/live-fire.sh` -> exit 1 because the host wrapper’s `archive-membership` and `multipart-media-ingestion` proofs cannot reach Docker-internal PostgreSQL/MinIO ports; the other fourteen proofs passed. Those two proofs were rerun successfully inside the internal network, including authenticated storage multipart behavior; the wrapper remains honestly ungreen until its runner is network-aware.
