...
- `sh scripts/restore-check.sh .artifacts/backups/family-historian-20260808T124930Z.dump.enc` -> `restore-check: ok`; the encrypted backup decrypted into a disposable database and reported `schema_migrations=11`.
- `sh scripts/performance-smoke.sh` -> `performance: ok requests=100 p95=0.54ms` against the real Fastify health endpoint.
