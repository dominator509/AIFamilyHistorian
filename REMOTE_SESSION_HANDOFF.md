...
- `sh scripts/backup.sh` -> `backup: ok`; a streaming AES-256-GCM encrypted PostgreSQL custom-format backup and SHA-256 sidecar were created under ignored `.artifacts/` (`family-historian-20260808T134608Z.dump.enc`).
- `sh scripts/restore-check.sh .artifacts/backups/family-historian-20260808T134608Z.dump.enc` -> `restore-check: ok`; the encrypted backup decrypted into a disposable database and reported `schema_migrations=12`.
- `sh scripts/performance-smoke.sh` -> `performance: ok requests=100 p95=0.56ms` against the real Fastify health endpoint.
