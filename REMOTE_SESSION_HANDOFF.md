...
- Current code checkpoint: `541ceaf` (hardening pass nine lease-fencing checkpoint; full commit is recorded in Git).
- Branch: `master`
...
- `sh scripts/test-integration.sh` -> `integration tests: ok` (5 files, 12 tests, including real SQL outbox claim/lease/completion/retry/dead-letter, unsupported-job, and stale-worker fencing proofs)
