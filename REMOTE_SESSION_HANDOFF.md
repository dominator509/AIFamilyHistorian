...
- Current code checkpoint: `HARDENING-23 pending commit` (privacy/export/narration review intake, Redis limiter, API authorization/publication/MIME hardening, bounded worker derivatives, authenticated principal/archive rate scopes, production secret entropy validation, revocable bearer sessions, transactional active-upload quotas, archive queue-capacity enforcement, derivative recipe idempotency, archive partitioning, provider-authoritative billing state, strict CORS allowlisting, and durable signature-verified Stripe callback ingestion).
- Branch: `master`
...
- `sh scripts/restore-check.sh .artifacts/backups/family-historian-20260808T105114Z.dump.enc` -> `restore-check: ok`; the encrypted backup decrypted into a disposable database and reported `schema_migrations=10`.
