...
| Worker and job execution | Dispatcher, media quarantine, privacy-request, export, and narration intake paths implemented; fulfillment families pending | SQL `FOR UPDATE SKIP LOCKED` claim/lease, UUID lease-token fencing, tenant transaction scope, bounded retry/backoff/dead-letter, unsupported-job fail-closed behavior, streamed object download/fixity, media plan execution, derivative persistence, clean/error quarantine transitions, privacy review intake with deletion holds, and review-gated export/narration intake | 15 real-database integration tests; worker image build/tool probe; media executor unit coverage | Privacy fulfillment/export/narration/transcription/deletion execution families, hosted object-storage fixture, production queue topology and authenticated media fixture | Intake handlers intentionally stop at review-required states; unimplemented fulfillment job types remain fail-closed; media fixture/live-fire still needs a representative object in the worker image |
