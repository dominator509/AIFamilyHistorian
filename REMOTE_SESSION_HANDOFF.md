...
- `sh scripts/test-integration.sh` -> `integration tests: ok` (7 files, 14 tests, including real Redis distributed rate limiting, tenant-scoped privacy intake/deletion-hold evidence, and SQL outbox claim/lease/completion/retry/dead-letter, unsupported-job, and stale-worker fencing proofs)
...
| Worker and job execution | Dispatcher, media quarantine, and privacy-request intake paths implemented; fulfillment families pending | SQL `FOR UPDATE SKIP LOCKED` claim/lease, UUID lease-token fencing, tenant transaction scope, bounded retry/backoff/dead-letter, unsupported-job fail-closed behavior, streamed object download/fixity, media plan execution, derivative persistence, clean/error quarantine transitions, and privacy review intake with deletion holds | 14 real-database integration tests; worker image build/tool probe; media executor unit coverage | Privacy fulfillment/export/narration/transcription/deletion execution families, hosted object-storage fixture, production queue topology and authenticated media fixture | Unimplemented fulfillment job types terminal-fail safely; privacy intake intentionally stops at review-required state; media fixture/live-fire still needs a representative object in the worker image |
...
- The worker now has a real SQL dispatcher with lease-token fencing, a media quarantine handler, and privacy-request intake that records review evidence and deletion holds. Privacy fulfillment, export, transcription, narration, and deletion execution still require implementation and fixture proofs; unsupported job types fail closed rather than reporting success.
