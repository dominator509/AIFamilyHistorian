...
if [ -n "$internal_network" ]; then
  ...
  internal_redis=$(REDIS_URL="$REDIS_URL" node --input-type=module -e "const u=new URL(process.env.REDIS_URL); u.hostname='redis'; u.port='6379'; process.stdout.write(u.toString())")
  mount_args=''
